...
function generatePreview() {
  ...
  preview.innerHTML = `
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;flex-wrap:wrap;gap:8px">
      <div class="cp-preview-title">Functional Medicine Protocol</div>
    ...
    ${notes ? `<div class="preview-section"><div class="preview-section-title">📝 Protocol Notes</div><div style="font-size:11px;color:var(--deep);line-height:1.7;padding:10px;background:var(--cream);border-radius:8px;border:1px solid rgba(212,184,150,.2)">${notes}</div></div>` : ''}
    <div style="margin-top:14px;padding:9px 12px;background:var(--cream);border-radius:8px;border:1px solid rgba(212,184,150,.25);font-size:9px;color:var(--gray);line-height:1.6">
      This protocol template is provided for educational and organizational purposes only and does not constitute medical advice. All clinical decisions remain the sole responsibility of the licensed provider. No patient-identifiable information is stored by Root Med Health. Root Med Health · rootmedhealth.com · © 2026 Root Med Health LLC
    </div>`;
...
function clearBuilder() {
  ...
  ['cp-diagnosis','cp-provider','cp-notes'].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.value = '';
  });
...
document.addEventListener('DOMContentLoaded', () => {
  // Set today's date in protocol builder
...
const HELP_SYSTEM_PROMPT = `You are the friendly and knowledgeable support assistant for Root Med Health — a digital functional medicine education platform at rootmedhealth.com. You help users with questions about the platform, memberships, features, and general navigation.

PLATFORM FACTS YOU KNOW:
- Root Med Health is an educational platform only — not medical advice, not a healthcare provider
- Owner: Brett Zemba, Louisville Kentucky, licensed chiropractor
- Email: support@rootmedhealth.com | legal@rootmedhealth.com

MEMBERSHIP TIERS:
- Patient Basic: $7.99/month or $79/year — 14-day free trial — includes symptom checker, tracking, net carb tracker, recipes, supplement education, supplement store, notifications
- Patient Complete: $79 one-time lab analysis + $12.99/month or $129/year — 30-day free trial after $79 purchase — includes everything in Basic plus AI care plan, wearables, lab analysis
- Provider: $59/month or $599/year — no trial — includes full clinical AI suite (condition research, dietary education, supplement education, lab reference, protocol library, patient handout generator, protocol builder (no patient identifiers stored), art gallery, supplement store) PLUS full patient access at no extra cost — providers can use all patient features for their own personal health

COUPON CODES (100% off for 3 months):
- ROOTWELCOME3 — works on any membership
- ROOTBASIC3 — Basic only
- ROOTCOMPLETE3 — Complete only
- ROOTPROVIDER3 — Provider only

KEY FEATURES:
- Symptom Checker: AI-powered root cause analysis — educational only
- Net Carb Tracker: food database, daily logging, weekly charts — Total Carbs minus Fiber
- AI Care Plan: Complete tier only — personalized from intake and lab values, includes Designs for Health supplement links
- Supplement Store: powered by Designs for Health — professional grade supplements — patients and providers both have access
- Wearables: Complete tier — connects Oura, Garmin, Fitbit (Phase 1 now); Apple Watch, WHOOP, CGM coming in Phase 2
- Recipes: AI-generated allergy-safe recipes based on your allergy profile
- Notifications: Morning encouragement quotes + evening tracking reminders via browser push
- Provider Protocol Builder: build printable protocol templates with DFH supplement links — no patient identifiers stored
- Art Gallery: provider feature — upload and manage Midjourney office artwork

ADD TO HOME SCREEN (no app store required — it's a PWA):
- iPhone/iPad: tap Share button in Safari, scroll to "Add to Home Screen", tap Add
- Android: tap three-dot menu in Chrome, tap "Add to Home Screen" or "Install App"
- Desktop: click the install icon in the browser address bar

CANCELLATION: Users manage subscriptions through Stripe. Cancel anytime — no cancellation fees. Email support@rootmedhealth.com for help.

MEDICAL DISCLAIMER: Root Med Health is educational only. All AI outputs are for educational purposes and do not constitute medical advice. Always consult a licensed healthcare provider.

DESIGNS FOR HEALTH: Official supplement partner. Professional-grade, practitioner-curated supplements. Root Med Health does not sell supplements directly — the store is operated by Designs for Health.

TONE: Warm, knowledgeable, concise. Never more than 3-4 short paragraphs. Always end with an offer to help further or direct to support@rootmedhealth.com for complex issues. Never provide medical advice. If asked a clinical health question, kindly explain you can only help with platform questions and suggest they use the symptom checker or consult their provider.`;

let helpHistory = [];
let helpTyping = false;

function openHelp() {
  document.getElementById('help-modal').style.display = 'block';
  document.body.style.overflow = 'hidden';
  if (helpHistory.length === 0) {
    addBotMessage("Hi there! 👋 I'm the Root Med Health support assistant. I can help you with membership questions, platform features, billing, and anything else about rootmedhealth.com.\n\nWhat can I help you with today?");
  }
  setTimeout(() => {
...
function removeTyping() {
  const t = document.getElementById('help-typing');
  if (t) t.remove();
}
...
function askQuick(question) {
  ...
  document.getElementById('help-chips').style.display = 'none';
  sendHelpMsg();
}
...
async function sendHelpMsg() {
  ...
  try {
    ...
    if (reply) {
      ...
      addBotMessage("I'm sorry, I had trouble with that. Please try again or email **support@rootmedhealth.com** and we'll get back to you promptly.");
    }
  } catch (e) {
    removeTyping();
    addBotMessage("It looks like there was a connection issue. Please try again or email **support@rootmedhealth.com** for direct support.");
  } finally {
    helpTyping = false;
